...
from simplestruct.field_types import integer, empty


class EmptySimpleStruct(SimpleStruct):
    int_a = integer(signed=False, endianness=Big)
    empty_space = empty(size=3)
    int_b = integer(signed=False, endianness=Big)


class TestEmptySimpleStruct(TestCase):
    def test_create(self):
        instance = EmptySimpleStruct(int_a=0xFFFFFFFF, int_b=0x01020304)
        assert "\xFF\xFF\xFF\xFF\x00\x00\x00\x01\x02\x03\x04" == \
            instance.serialize()

    def test_parse(self):
        instance = EmptySimpleStruct.parse_from(
